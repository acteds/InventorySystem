...
<center>
	<div class="c1"><%--@elvariable id="user" type="java.util.Map"--%>
		<h2>出库清单管理</h2><%--@elvariable id="sum" type="int"--%>
		<table border="1" class="table table-striped table-bordered table-condensed table-hover">
			...
			<c:forEach var="temp" items="${list}">
				<tr><%--@elvariable id="goodsMap" type="java.util.LinkedHashMap"--%>
					...
					<td align="center">
							<%--审核通过后无法执行删除与修改操作--%>
						<c:if test="${temp.get(\"review\")!=10}">
							<a onClick="return deleteDemo(`${goodsMap.get(temp.get('gid').toString())}`)" href="inventorySubDel?iid=${temp.get('iid')}" class="fonts4">删除</a>&nbsp;|&nbsp;
							<a href="inventorySubChange?iid=${temp.get('iid')}">修改</a>
						</c:if>
							<%--审核未通过--%>
						<c:if test="${temp.get(\"review\")==1}">
							&nbsp;|&nbsp;<a href="inventoryReviewSubInfo?iid=${temp.get('iid')}">确认消息</a>
						</c:if>
							<%--审核通过--%>
						<c:if test="${temp.get(\"review\")==10}">
							<a href="inventoryReviewSubInfo?iid=${temp.get('iid')}">确认消息</a>
			...
			<tr><td colspan="${fn:length(top)+1}" align="center"><a class="fonts1" href="inventorySubInsertList">添加货品出库清单</a></td></tr>
		</table>
	</div>
</center>
