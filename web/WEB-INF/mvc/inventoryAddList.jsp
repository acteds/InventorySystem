...
<center>
	<div class="c1"><%--@elvariable id="user" type="java.util.Map"--%>
		<h2>入库清单管理</h2><%--@elvariable id="sum" type="int"--%>
			...
			<c:forEach var="temp" items="${list}">
				<tr><%--@elvariable id="goodsMap" type="java.util.LinkedHashMap"--%>
					...
					<td align="center">
						...
						<a onClick="return deleteDemo(`${goodsMap.get(temp.get('gid').toString())}`)" href="inventoryAddDel?iid=${temp.get('iid')}" class="fonts4">删除</a>&nbsp;|&nbsp;
						<a href="inventoryAddChange?iid=${temp.get('iid')}">修改</a>
						</c:if>
						<%--审核未通过--%>
						<c:if test="${temp.get(\"review\")==1}">
							&nbsp;|&nbsp;<a href="inventoryReviewAddInfo?iid=${temp.get('iid')}">确认消息</a>
						</c:if>
						<%--审核通过--%>
						<c:if test="${temp.get(\"review\")==10}">
							<a href="inventoryReviewAddInfo?iid=${temp.get('iid')}">确认消息</a>
