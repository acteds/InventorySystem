...
<html><%--@elvariable id="translate" type="java.util.Map"--%>
<head><%--@elvariable id="assnChange" type="java.util.Map"--%>
...
<center>
<div class="message">
<form name="form1" action="AssnChange" method="post" onsubmit="return NotNull(this);">
    <table width="317"><%--@elvariable id="top" type="java.lang.String"--%>
        <tr><td colspan="2" class="title">${assnChange.assn_name}信息修改</td></tr>
        ...
        <c:forEach var="st" items="${top}">
            <c:if test="${st== 'assn_content'||st=='assn_brief'||st=='assn_address'}" var="if1" scope="page">
                <tr>
                    ...
                    <td><textarea style="width: 166px;" name="${st}">${assnChange.get(st)}</textarea></td>
                </tr>
            </c:if>
            <c:if test="${not if1}">
                <tr>
                    <td align="right">${translate.get(st)}:</td>
                    <td><input name="${st}" type="text" style="width: 167px;" value="${assnChange.get(st)}">
                    </td>
                </tr>
            </c:if>
        </c:forEach>
