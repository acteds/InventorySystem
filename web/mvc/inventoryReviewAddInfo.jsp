        ...
        <c:forEach var="st" items="${top}">
            ...
            <c:if test="${st== 'review'}" var="if5" scope="page">
                <tr>
                    ...
                    <td>
                        <input name="${st}" type="text" class="underline-border" style="width: 167px;"
                               value="${parameters.get("3").get(list.get(st).toString())}" readonly="readonly">
                    </td>
                </tr>
                <tr><%--@elvariable id="userMap" type="java.util.Map"--%>
                    <td align="right">审核员:</td>
                    <td><input name="uid2" type="text" class="underline-border" style="width: 167px;"
                               value="${userMap.get(list.get("uid2").toString())}" readonly="readonly">
                    </td>
                </tr>
                <tr>
                    <td align="right">审核说明:</td>
                    <td><textarea style="width: 166px;" name="explanation2"  readonly="readonly" class="fonts2">${list.get("explanation2")}</textarea></td>
                </tr>

            </c:if>
            <c:if test="${not if1 and not if2 and not if3 and not if4 and not if5 and st!= 'explanation2' and st!='uid2'}">
                <tr>
                    <td align="right">${translate.get(st)!=null?translate.get(st):st}:</td>
