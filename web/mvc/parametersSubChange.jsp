...
<form name="form1" action="ParametersSubChange" method="post" onsubmit="return NotNull();">
    <table width="317">
        <tr><td colspan="2" class="title">修改键值对</td></tr>
        <tr><td width="100" align="right"></td><td width="217"></td>
        </tr>
        <c:forEach var="st" items="${top}">
            <c:if test="${st== 'explanation'}" var="if1" scope="page"><%--@elvariable id="parametersMain" type="java.util.LinkedHashMap"--%>
                <tr>
                    <td align="right">${translate.get(st)}:</td>
                    <td><textarea style="width: 166px;" name="${st}">${parametersSub.get(st)}</textarea></td>
                </tr>
            </c:if>
            <c:if test="${st=='psid'}" var="if2" scope="page">
                <tr>
                    <td align="right">${translate.get(st)}:</td>
                    <td><input name="${st}" type="text" style="width: 167px;" value="${parametersSub.get(st)}" disabled="disabled"></td>
                </tr>
            </c:if>
            ...
            <c:if test="${not if1 and not if2 and not if3}">
                <tr>
                    <td align="right">${translate.get(st)}:</td>
                    <td><input name="${st}" type="text" style="width: 167px;" value="${parametersSub.get(st)}"></td>
                </tr>
            </c:if>
        </c:forEach>
        <tr>
