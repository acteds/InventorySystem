        ...
        <c:forEach var="st" items="${top}">
            <c:if test="${st== 'explanation'}" var="if1" scope="page"><%--@elvariable id="parametersMain" type="java.util.LinkedHashMap"--%>
                <tr>
                    ...
                    <td><textarea style="width: 166px;" name="${st}">${parametersMain.get(st)}</textarea></td>
                </tr>
            </c:if>
            <c:if test="${st=='pmid'}" var="if2" scope="page">
                <tr>
                    <td align="right">${translate.get(st)}:</td>
                    <td><input name="${st}" type="text" style="width: 167px;" value="${parametersMain.get(st)}" disabled="disabled"></td>
                </tr>
            </c:if>
            <c:if test="${not if1 and not if2}">
                <tr>
                    <td align="right">${translate.get(st)}:</td>
                    <td><input name="${st}" type="text" style="width: 167px;" value="${parametersMain.get(st)}"></td>
                </tr>
            </c:if>
        </c:forEach>
        <tr>
